(function umd(root, factory) {
     ...
     * @param {number} options.id - Price id.
     *
     ...
     *
     * @param {boolean} [options.isTest] - is Test.
     * @alias Seatchart
     */
    function Seatchart(options, cartA, bui=false) {
        /**
        ...
        } else if (options.map.rows > 500 || options.map.columns > 500) {
            throw new Error("Invalid parameter 'options.map' supplied to Seatchart. " +
        ...
        */
        ...
        var alphabet = []
        for(i = 0; alpha.length > i ; i++){
            alphabet.push({letter: alpha[i]})
        }
        if(options.map.rows > 25 ){
            for(var i = 26; options.map.rows > i ; i++){
                const number = Math.trunc((i/26) + 1)
                let letter = ""
                for(var j = 0; number > j; j++){
                    letter += String.fromCharCode(97 + (i % 26)).toUpperCase()
                }
        ...
        var updateTotal = function updateTotal() {
            if (cartTotal !== undefined) {
                cartTotal.textContent = 'Total: {0}{1}'.format(options.cart.currency, self.getTotal().toFixed(2).replace('.', ','));
            }
        ...
        var createCartItem = function createCartItem(seat) {
            ...
            seatPrice.textContent = '{0}{1}'.format(seat.price.toFixed(2).replace('.', ','), options.cart.currency);
        ...
        var updateCart = function updateCart(action, id, type, previousType, emit) {
            ...
            if (action === 'remove') {
                ...
                if (emit && self.onChange !== null && options.isTest) {
                    self.onChangeTest({
                        action: action,
                        current: current,
                        previous: previous
                    });
                } else if(emit && self.onChange !== null){
                    self.onChange({
                        action: action,
                        current: current,
                        previous: previous
                ...
                if (emit && self.onChangeTest !== null && options.isTest) {
                    self.onChangeTest({
                        action: action,
                        current: current,
                        previous: previous
                ...
                if (cartTable) {
                    ...
                    ticketPrice.textContent = '{0}{1}'.format( current.price.toFixed(2));
                }
        ...
        var seatClick = function seatClick() {
            ...
            var typesArr = types.map(item => item.replace(/ /g, ''))
            ...
            for (var i = 0; i < currentClassList.length; i += 1) {
                ...
                var newClass;
                if(currentClass == "btn-g"){
                    var index = typesArr.indexOf(currentClass);
                    
                    
                    index = options.types.findIndex(i => i.type.replace(/ /g, '') == typesArr[1].replace(/ /g, ''))
                    if (index !== -1) {
                             
                        this.style.backgroundColor = options.types[index].backgroundColor;
                        this.style.color = options.types[index].textColor;
                    }
                    this.classList.remove('btn-g');
                    this.classList.add('clicked');
                    updateCart('remove', this.id, typesArr[1].replace(/ /g, ''), currentClass, true);
                }
                if (currentClass !== 'sc-seat' && currentClass !== 'clicked') {
                    ...
                    if (index !== -1) {
                        if(options.isTest){
                            this.classList.remove(typesArr[index].replace(/ /g, ''));
                        }
                        index += 1;
                        ...
                        var cl
                        cl = options.types.find(i => i.type.replace(/ /g, '') == typesArr[1].replace(/ /g, '')).type.replace(/ /g, '')
                        

                        this.style.backgroundColor = '';
                        this.style.color = '';
                        this.classList.add(cl ? cl.replace(/ /g, '') : newClass.replace(/ /g, ''));
  
                        // if the class isn't available then apply the background-color in the config
                        if (newClass !== 'available') {
                            if(typeof options.id == "number"){
                                newClass = options.types.find(i => i.id == options.id).type.replace(/ /g, '')                                
                            }
                            ...
                            if(options.isTest && typeof options.id == "number"){
                                index = options.types.findIndex(i => i.id == options.id)
                            }
                        ...
                        if (currentClass === 'available') {
                            ...
                            } else if(options.isTest && typeof options.id == "number"){
                                updateCart('add', this.id, newClass, currentClass, true);
                            }
                        } else if (newClass === 'available') {
                            if(!options.isTest){
                                updateCart('add', this.id, currentClass, currentClass, true);
                                this.classList.add('btn-g');
                            } else {
                        ...
                        } else if (removeFromCartDict(this.id, currentClass)) {
                            updateCart('update', this.id, newClass, currentClass, true);
                        } else if(currentClass != newClass && options.isTest && currentClass != 'available'){
                            updateCart('update', this.id, newClass, currentClass, true);
                        }
                    }
                }
        ...
        var rowName = function rowName(index, disabled, disabledCount) {
            if (!disabled) {
                return alphabet[index - disabledCount].letter;
            }
        ...
        var removeAllTypesApplied = function removeAllTypesApplied(seat) {
            for (var i = 0; i < types.length; i += 1) {
                seat.classList.remove(types[i].replace(/ /g, ''));
            }
        };
                ...
                if ({}.hasOwnProperty.call(seatType, 'selected') && seatType.selected) {
                    ...
                    for (var l = 0; l < seatType.selected.length; l += 1) {
                        ...
                        if (element) {
                            ...
                            element.classList.add(type.replace(/ /g, ''));
        ...
        this.set = function set(index, type, emit) {
            ...
            element.classList.add(classes[type].replace(/ /g, ''));
            element.classList.remove(classes[seat.type].replace(/ /g, ''));
        ...
        * @listens ChangeEvent
        */
        this.onChangeTest = function onChangeTest(action, current, previous){
            if(action.action == "add" && typeof options.id == "number"){
                let b = options.types.find(i => i.id == options.id)
                const index = options.types.findIndex(i => i.id == options.id)
                if(b != undefined){
                    options.types[index].selected.push(action.current.index)
                }
            } 
            if(action.action == "update" && typeof options.id == "number"){
                let b = options.types.find(i => i.id == options.id)
                const index = options.types.findIndex(i => i.id == options.id)
                for(var i = 0; options.types.length > i; i++){
                    options.types[i].selected = options.types[i].selected.filter(item => item != action.current.index)                    
                }
                if(b != undefined){
                    options.types[index].selected.push(action.current.index)
                }
            ...
            if(action.action == "update" && action.current.type == undefined){
                for(var i = 0; options.types.length > i; i++){
                    options.types[i].selected = options.types[i].selected.filter(item => item != action.current.index)                    
                }
            } 
            if(action.action == "remove" && typeof options.id == "number"){
                for(var i = 0; options.types.length > i; i++){
                    options.types[i].selected = options.types[i].selected.filter(item => item != action.current.index)                    
                }
        ...
        this.returnCart = function returnCart(){
            return cartA;
        };

        this.returnSeats = function returnSeats(){
            return options.map.disabled.seats;
        };
         /**
        * Triggered when a seat is selected or unselected.
        *
        ...
        * @listens ChangeEvent
        */
          this.onChange = function onChange(action, current, previous){
            if(action.action == "add"){
                const index = cartA.findIndex(item => item?.type == action.previous.type)
                if(0 > index){
                    cartA.push({seats: [action.previous.index], type: action.previous.type})
                } else {
                    cartA[index].seats.push(action.previous.index)
                }
            } 
            ...
            if(action.action == "remove"){
                const index = cartA.findIndex(item => item?.type == action.current.type)
                cartA[index].seats = cartA[index].seats.filter(item => item != action.current.index)      
            }
        };
        ...
        var createLegendItem = function createLegendItem(content, type, backgroundColor) {
            ...
            item.className = 'sc-legend-item d-flex flex-row';
        ...
        var createCartTotal = function createCartTotal() {
            ...
            cartTotal = createSmallTitle('Total: {0}{1}'.format(self.getTotal().toFixed(2).replace('.', ','), options.cart.currency));
        ...
        var createLegend = function createLegend() {
            ...
            var legendTitle = createTitle('Légende');
            ...
            for (var i = 0; i < options.types.length; i += 1) {
                var description = '{0} {1}{2}'.format(
                    ...
                    options.types[i].price.toFixed(2).replace('.',','),
            ...
            if(options.isTest){
                seatsList.appendChild(createLegendItem('Sans tarif', 'available'));
            } else if(!options.isTest){
                seatsList.appendChild(createLegendItem('Place sélectionnée', 'btn-g'));
            }
            seatsList.appendChild(createLegendItem('Déjà réservée', 'unavailable'));
        ...
        var createMap = function createMap() {
            ...
            options.map.disabled.seats = [...options.map.disabled.seatsDsb]
        ...
        if (options.legend && options.legend.id && (!options.isTest || (options.isTest && !bui))) {
            createLegend();
        }
    }
    if(typeof window != 'undefined'){
        window.SeatchartJS = Seatchart
    }
    return Seatchart;
  })));
